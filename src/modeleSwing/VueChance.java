...
public class VueChance extends JDialog {
  ...
  public VueChance(JFrame j, JeuReel jeuReel) {
	  ...
	  text =  new JTextArea("Vous êtes tombé sur une case Chance.\nVeuillez piocher une carte Chance.\n"
			  + "Appuyez sur ok pour réaliser l'action.", 200, 150);
	  text.setEditable(false);
	  text.setBounds(120,70,300,50);
	  text.setBorder(null);
	  text.setOpaque(false);
	  text.setBackground(new Color(0,0,0,0));
	  ImageIcon icone = VueMonopoly.traitementImg("chance.jpg", label);//new ImageIcon("chance.jpg");
	  label.setIcon(icone);
  ...
  private class Piocher implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(first) {
				...
				CaseChance caseChance = (CaseChance) jeuReel.getPlateau().getCase(joueur.getPion().getNumeroCase());//
				caseChance.actionCase(jeuReel, joueur);
				String textcarte = caseChance.getCarteChance().getTexteCarte();
				text.setText("Vous avez tiré une carte Chance :\n"+
				textcarte);
				first =false;
				repaint();
				}else {
